...
def get_labour_by_id(db: SessionDep, cache: CacheDep, jwt: JwtDep, admin: AdminRoleDep, id: str):
    return labour_service.get_labour_by_id(db=db, cache=cache, id=id)

@router.post("", 
    summary="Create labour info", 
    responses={
        201: Response(
...
def update_indirect_cost(db: SessionDep, cache: CacheDep, jwt: JwtDep, admin: AdminRoleDep, id: str, body: UpdateLabour = Body()): # type: ignore
    return labour_service.update_labour(db=db, cache=cache, id=id, body=body)

@router.delete("/{id}", 
    summary="Delete labour info", 
    responses={
        200: Response(
...
def delete_labour(db: SessionDep, cache: CacheDep, jwt: JwtDep, admin: AdminRoleDep, id: str):
    return labour_service.delete_labour(db=db, cache=cache, id=id)
