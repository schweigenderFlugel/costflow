...
def get_indirect_cost_by_id(db: SessionDep, cache: CacheDep, id: str):
    try:
        cached_item = cache.get(f"indirect_cost_{id}")
...
def get_current_indirect_costs(db: SessionDep, cache: CacheDep):
    try:
        cached_list = cache.get("indirect_costs_list")
...
def create_indirect_cost(db: SessionDep, cache: CacheDep, body: CreateIndirectCost):
    try:
        this_year = datetime.now().year
        this_month = datetime.now().month
        ...
        if cached_list:
            cache.delete("indirect_costs_list")
...
def update_indirect_cost(db: SessionDep, cache: CacheDep, id: str, body: UpdateIndirectCost): # type: ignore
    try:
        this_year = datetime.now().year
        this_month = datetime.now().month
        ...
        if cached_item:
            cache.delete(f"indirect_cost_{id}")
...
def delete_indirect_cost(db: SessionDep, cache: CacheDep, id: str):
    try:
        indirect_cost_found = db.get(IndirectCost, id)
        if indirect_cost_found is None:
            raise HTTPException(status_code=404, detail="indirect cost not found")
