...
export const useIndirectCostMutations = () => {
  const createIndirectCost = useDataMutation({
    ...
    onSuccess: (data, variables) => {
      const finalType = variables.type === "Otros" && variables.customType ? variables.customType : variables.type;
  ...
  const updateIndirectCost = useDataMutation({
    queryType: "indirect_cost",
    mutationFn: async ({ indirectCostId, ...values }: FormDataIndirectCost & { indirectCostId: string }) => {
      const data = await fetcher({
      ...
      if (data.error || !data.message?.includes("successfully")) {
        ...
        throw new Error(posibleMessage || "Error al actualizar el costo indirecto");
      }
  ...
  const deleteIndirectCost = useDataMutation({
    queryType: "indirect_cost",
    mutationFn: async ({ indirectCostId, indirectCostType }: { indirectCostId: string; indirectCostType: string }) => {
      const data = await fetcher({
        input: `/api/indirect_cost/${indirectCostId}`,
        method: "DELETE"
      });
      ...
      if (data.error || !data.message?.includes("successfully")) {
        ...
        throw new Error(posibleMessage || "Error al eliminar el costo indirecto");
      }
