...
const UpdateProduct = () => {
  ...
  const [updatedProductData, setUpdatedProductData] =
    useState<FormDataProduct | null>(null);
  const { isOpen, setIsOpen, product, setProduct, isLoadingProduct } =
      ...
      <SheetContent className="py-6 px-4 gap-1 justify-start sm:max-w-2xl">
        <SheetHeader className={alreadyUpdated ? "sr-only" : "px-0"}>
          ...
          <SheetDescription className="text-left">
            Actualizá la información de un producto ya cargado, modificando su
            nombre, cantidad, insumos o cualquier otro dato relevante para
