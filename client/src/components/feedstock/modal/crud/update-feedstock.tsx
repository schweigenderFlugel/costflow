...
const UpdateFeedstock = () => {
  const { isOpen, setIsOpen, feedstock, setFeedstock } =
    useUpdateFeedstockDialog();
  const [alreadyUpdated, setAlreadyUpdated] = useState<boolean>(false);
  const [updatedFeedstockData, setUpdatedFeedstockData] =
    useState<FormDataFeedstock | null>(null);
      ...
      <SheetContent className="py-6 px-4 gap-1 justify-start">
        <SheetHeader className={alreadyUpdated ? "sr-only" : " px-0"}>
          ...
          <SheetDescription className="text-left">
            Actualizá la información de un insumo ya cargado, modificando su
            nombre, cantidad, valor o cualquier otro dato relevante para
            mantener tus registros siempre correctos y actualizados.
          </SheetDescription>
        </SheetHeader>
