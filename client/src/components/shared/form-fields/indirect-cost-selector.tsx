...
import { IndirectCostSelectorProps } from "@/interfaces/interface-indirect-cost-selector-props";
...
export function IndirectCostSelector({
  ...
  const { data: indirectCosts = [], isLoading } = useDataQuery<
    IndirectCostObj[]
  >("indirect_cost", undefined);

  const [filteredIndirectCosts, setFilteredIndirectCosts] = useState<
    IndirectCostObj[]
  >([]);
  ...
  const getIndirectCostInfo = (indirectCostId: string) => {
    ...
    return indirectCosts.find(
      (ic: IndirectCostObj) => ic.id === indirectCostId
    );
  };
  ...
  useEffect(() => {
    ...
    const addedIndirectCostIds = (watchedIndirectCosts || [])
      .map((item) => item?.id)
      .filter(Boolean);

    filtered = filtered.filter(
      (ic: IndirectCostObj) => !addedIndirectCostIds.includes(ic.id)
    );
  ...
  const handleIndirectCostSelect = (indirectCost: IndirectCostObj) => {
    ...
    const alreadySelected = (watchedIndirectCosts || []).some(
      (item) => item?.id === indirectCost.id
    );
  ...
  return (
    <div className="col-span-1 space-y-4 h-full">
      <div className="space-y-2">
        <Label htmlFor="indirect-cost-search">
          Costos indirectos vinculados
        </Label>
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Input
                      ...
                      step="1"
                      className="pr-12"
                      {...formRegister(`indirect_costs.${index}.usage`, {
                        valueAsNumber: true,
                      })}
                    />
                  </div>
      ...
      {fields.length === 0 && (
        <div className="text-center py-6 px-2 text-gray-500 border-2 border-dashed border-gray-200 rounded-lg">
          ...
          <p className="text-xs">
            Usa el buscador para agregar costos indirectos al producto
          </p>
        </div>
      )}
    </div>
