...
import { LabourFormProps } from "@/interfaces/interface-labour-form-props";

const LabourForm = ({
  type = "create",
  onCancel,
  initialValues = { hours: undefined, salary: undefined, date: new Date() },
  isDisabled = false,
}: LabourFormProps) => {
  ...
  return (
    <form
      ...
      className={`flex flex-col gap-5 justify-between p-3 bg-background border rounded-xl ${
        type === "create" ? "mt-4 " : "bg-muted"
      }`}
    >
      {type === "create" && (
        <h3 className="text-lg font-medium">Agregar nuevo salario</h3>
      )}
        ...
        <div className="gap-3 grid grid-cols-2 sm:grid-cols-4 w-full">
          <label className="w-full text-sm">
            ...
            <input
              ...
              placeholder="Puesto"
              className={`${
                type === "create" ? "outline" : "bg-background"
              } rounded-lg w-full px-3 py-1.5 my-1 text-base`}
              value={"Operario"}
              disabled
            />
          </label>
