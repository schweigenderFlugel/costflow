...
export const feedstockSchema = z
  .object({
    name: z
      .string()
  ...
  .refine(
    (data) => {
      ...
      if (data.state === StateMatter.SOLID) {
        return Object.values(SolidMeasure).includes(
          data.measure_unit as unknown as SolidMeasure
        );
      }
      if (data.state === StateMatter.LIQUID) {
        return Object.values(LiquidMeasure).includes(
          data.measure_unit as unknown as LiquidMeasure
        );
      }
      if (data.state === StateMatter.GASEOUS) {
        return Object.values(GasMeasure).includes(
          data.measure_unit as unknown as GasMeasure
        );
      }
      return false;
    },
    {
      message:
        "La unidad de medida no es válida para el estado de la materia seleccionado",
      path: ["measure_unit"],
    }
  );
...
export const objFeedstockSchema = feedstockSchema.extend({
  ...
  created_at: z.date({
    error: () => ({ message: "Fecha de creación inválida" }),
  }),
  updated_at: z.date({
    error: () => ({ message: "Fecha de actualización inválida" }),
  }),
});
